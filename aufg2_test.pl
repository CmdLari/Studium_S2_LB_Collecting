...
test(collect_stueck) :-
    ...
    collect_stueck(Collection),write('>'),
    Collection = [('Concerto grosso op.6 Nr.7', 'B-dur'),
...
test(collect_gs) :-
    write('collect_gs '),
    collect_gs(Collection),write('>'),
    Collection = [(103, 100)|Rest],
    length(Rest,5),write('.'),
...
test(collect_st) :-
    write('collect_st '),
    collect_st(['Beethoven','Händel'],Collection),write('>'),
    Collection = [('Beethoven', 'Die Geschöpfe des Prometheus, Ouvertüre'),
...
test(collect_komp) :-
    ...
    collect_komp('Violine',Collection),write('>'),
    Collection = [('Serge', 'Prokofiev')|Rest],
    length(Rest,6),write('.'),
...
test(collect_time) :-
    write('collect_time '),
    collect_time(Collection),write('>'),
    Collection = [('Georg Friedrich','Händel',455)|Rest],
